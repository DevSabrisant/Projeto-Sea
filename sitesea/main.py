...
def MudarVen(vAtual, vNovo,vPlano,checkA):
    ...
    if not checkA:
        ...
        # VENCIMENTO 25 OU 30
            ...
            Valor = Qtd * globals()[f"plan{vPlano}"]()
            ValorDiferenca = (Qtd-30)*-1 * globals()[f'plan{vPlano}']()
            r = f"Plano: {vPlano}Megas\nVencimento: {vAtual} para {vNovo}\n\n{IniVenc21Br} -- {FinalVenc10Br}. São {Qtd} dias -- totalizando: {Valor:.2f}\nCom desconto de 10%: {Valor - Valor * 0.1:.2f}\nDesconto de: {Valor * 0.1:.2f}\n\nVencimento mais proximo:\n{IniVenc11Br} -- {date(data_hoje.year,data_hoje.month,20).strftime('%d/%m/%Y')}. São {(Qtd-30)*-1} dias -- totalizando:  {ValorDiferenca:.2f}\nCom desconto de 10%: {Valor - Valor * 0.1:.2f}\nDesconto de: {Valor * 0.1:.2f}  1" if ValorDiferenca > 50                                           else f"Plano: {vPlano}Megas\nVencimento: {vAtual} para {vNovo}\n\n{IniVenc01Br} -- {FinalVenc10Br}. São {Qtd} dias -- totalizando: {Valor:.2f}\nCom desconto de 10%: {Valor - Valor * 0.1:.2f}\nDesconto de: {Valor * 0.1:.2f}\n\nProporcional de {(Qtd-30)*-1} dias: {ValorDiferenca:.2f}\n\n{MensagemFatura}"
            return r
            ...
            Valor = Qtd * globals()[f"plan{vPlano}"]()
            ValorDiferenca = (Qtd-30)*-1 * globals()[f'plan{vPlano}']()
            r = f"Plano: {vPlano}Megas\nVencimento: {vAtual} para {vNovo}\n\n{IniVenc21Br} -- {FinalVenc30Br}. São {Qtd} dias -- totalizando: {Valor:.2f}\nCom desconto de 10%: {Valor - Valor * 0.1:.2f}\nDesconto de: {Valor * 0.1:.2f}\n\nVencimento mais proximo:\n{IniVenc21Br} -- {date(data_hoje.year,data_hoje.month,20).strftime('%d/%m/%Y')}. São {(Qtd-30)*-1} dias -- totalizando:  {ValorDiferenca:.2f}\nCom desconto de 10%: {Valor - Valor * 0.1:.2f}\nDesconto de: {Valor * 0.1:.2f}  1" if ValorDiferenca > 50                                                  else f"Plano: {vPlano}Megas\nVencimento: {vAtual} para {vNovo}\n\n{IniVenc01Br} -- {FinalVenc10Br}. São {Qtd} dias -- totalizando: {Valor:.2f}\nCom desconto de 10%: {Valor - Valor * 0.1:.2f}\nDesconto de: {Valor * 0.1:.2f}\n\nProporcional de {(Qtd-30)*-1} dias: {ValorDiferenca:.2f}\n\n{MensagemFatura}"
            return r
    else:
